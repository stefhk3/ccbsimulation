...
public class GraphChecks {
	DefaultDirectedGraph<Process, DefaultEdge> graph;
    ...
    private void followProcess(Process p, DefaultDirectedGraph<Process, DefaultEdge> graph){
        if(p==null)
            return;
        else{
            if(p instanceof Nil)
                ...
                followProcess(((Parallel)p).getLeft(), graph);
                followProcess(((Parallel)p).getRight(), graph);
            }else if(p instanceof Prefix){
                graph.addVertex(p);
                followProcess(((Prefix)p).getProcess(), graph);
                ...
                followProcess(((Restriction)p).getProcess(), graph);
            }
        }
    }
     ...
     * @throws CCBException 
     */
    private DefaultDirectedGraph<Process, DefaultEdge> createGraph(Process p) throws CCBException
    {
        DefaultDirectedWeightedGraph<Process, DefaultEdge> graph = new DefaultDirectedWeightedGraph<Process, DefaultEdge>(DefaultEdge.class);
     ...
     * @return
     */
    public DefaultDirectedGraph<Process, DefaultEdge> getGraph() {
        return graph;
    }
    ...
    public boolean isIsomorph(Process p2) throws CCBException{
    	DefaultDirectedGraph<Process, DefaultEdge> g2=createGraph(p2);
